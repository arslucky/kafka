 ...
 *
 * ./kafka-topics.sh --bootstrap-server kafka1:9091 --create --partitions 1 --topic KGlobalTablePartitions1Product
 * ./kafka-topics.sh --bootstrap-server kafka1:9091 --create --partitions 2 --topic KGlobalTablePartitions1Sale
 */
public class KGlobalTablePartitions1 {

    static Logger log = LogManager.getLogger( KGlobalTablePartitions1.class);

    static String topic_product = KGlobalTablePartitions1.class.getSimpleName() + "Product";

    static String topic_sale = KGlobalTablePartitions1.class.getSimpleName() + "Sale";
        ...
        public void run() {
            ...
            try {
                for( int i = 1; i < 4; i++) {
                    ...
                    Future<RecordMetadata> res = producerProduct.send( record);
                    log.info( "send product key:{}, value:{}, partition:{}", keyProduct, value, res.get().partition());

                    ProducerRecord<Integer, Integer> recordSale = new ProducerRecord<>( topic_sale, keySale, keyProduct);
                    res = producerSale.send( recordSale);
                    log.info( "send sale key:{}, key product:{}, partition:{}", keySale, keyProduct, res.get().partition());
                }
            } catch( Exception e) {
                e.printStackTrace();
            } finally {
    ...
    static class Consumer implements Runnable {
        ...
        public Consumer( int index) {
            String baseDir = String.format( "./kafka-streams/%s/%s", KGlobalTablePartitions1.class.getSimpleName(), index);
        ...
        public void run() {
            try {
                ...
                stream.foreach( ( key, value) -> {
                    log.info( "{} sale key:{}, product key:{}", currentThread().getName(), key, value);
                });

                //@formatter:off
                KStream<Integer, String> streamJoined = stream.leftJoin( globalTable,
                        (streamKey, streamValue) -> streamValue /* streamValue == globalTable.key */,
                        (streamValue, globalTableValue) -> String.format( "product:%s", globalTableValue));
                //@formatter:on

                streamJoined.foreach( ( key, value) -> {
                    log.info( "{} streamJoined key:{}, value:{}", currentThread().getName(), key, value);
                });
