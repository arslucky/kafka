 ...
 *
 * ./kafka-topics.sh --bootstrap-server localhost:9091 --create --topic ProducerTransaction1 --replication-factor 2
 */
public class ProducerTransaction1 {

    static Logger log = LogManager.getLogger( ProducerTransaction1.class);

    private static final String TIME_FORMAT = "kk:mm:ss.SSS";
    static String topic = ProducerTransaction1.class.getSimpleName();
        ...
        public void run() {
            try {
                ...
                for( int i = 0; i < 5; i++) {
                    ...
                    String value = String.format( "%s %s", "commited", dateFormat.format( new Date( ms)));
            ...
            try {
                ...
                while( !closed.get()) {
                    ...
                    for( ConsumerRecord<Integer, String> record : records) {
                        log.info( String.format( "get:%d %s %d", record.key(), record.value(), record.offset()));
                    }
                }
            } catch( Exception e) {
                e.printStackTrace();
